    ...
    <PageContainer className="py-8">
      ...
      <section className="mt-10">
        <div className="mb-4 flex items-center justify-between">
          <h2 className="text-[32px] font-semibold tracking-normal text-[#111111]">소속 디자이너</h2>
          <span className="text-sm text-[#7b7368]">총 {designers.length}명</span>
        </div>

        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">
          {designers.map((designer) => (
            <article key={designer.id} className="overflow-hidden rounded-[16px] border border-[#dfd8ce] bg-white">
              <div className="relative h-56 bg-[#eee8df]">
                <Image src={designer.profileImage} alt={designer.name} fill className="object-cover object-top" sizes="(max-width:1024px) 50vw, 25vw" />
              </div>
              <div className="p-4">
                <div>
                  <p className="text-[24px] leading-tight font-semibold tracking-normal text-[#111111]">{designer.name}</p>
                  <p className="mt-1 text-sm text-[#6f675d]">{designer.styles.slice(0, 2).join(" · ")}</p>
                  <p className="mt-2 text-sm text-[#5f5951]">평점 {designer.rating} · 리뷰 {designer.reviewCount}</p>
                </div>
      ...
      <section className="mt-10">
        <h2 className="mb-4 text-[32px] font-semibold tracking-normal text-[#111111]">대표 스타일</h2>
          ...
          {styles.map((style) => (
            <article key={style.rank} className="overflow-hidden rounded-2xl border border-[#dfd8ce] bg-white">
              ...
              <div className="p-4">
                ...
                <p className="text-[26px] font-semibold tracking-normal text-[#111111]">{style.keyword}</p>
              </div>
            </article>
          ))}
        </div>
      ...
      <section className="mt-10 rounded-2xl border border-[#ded7cd] bg-white p-6">
        <h2 className="text-[32px] font-semibold tracking-normal text-[#111111]">리뷰</h2>
      ...
      <section className="mt-10 rounded-2xl border border-[#d8d1c6] bg-[#efe9df] p-6">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <h3 className="text-[32px] font-semibold tracking-normal text-[#111111]">외부 예약으로 바로 연결</h3>
