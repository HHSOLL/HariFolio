...
export function SalonHero({ salon }: SalonHeroProps) {
  ...
  return (
    <section className="grid gap-5 xl:grid-cols-[1.05fr_1fr_0.8fr]">
      <article className="overflow-hidden rounded-3xl border border-[#dfd7cc] bg-white">
        <div className="relative h-[420px]">
          ...
          <button
            type="button"
            className="absolute bottom-4 right-4 rounded-xl bg-[#111111b8] px-3 py-2 text-sm font-medium text-white"
          >
            사진 더보기 20+
          </button>
        </div>
      ...
      <article className="rounded-3xl border border-[#dfd7cc] bg-white p-8">
        ...
        <h1 className="mt-4 text-[56px] leading-tight font-semibold tracking-[-0.03em] text-[#111111]">{salon.name}</h1>
        <p className="mt-2 text-xl text-[#4f493f]">★ {salon.rating} ({salon.reviewCount.toLocaleString()}) · 리뷰 {salon.reviewCount.toLocaleString()}</p>
      ...
      <aside className="rounded-3xl border border-[#dfd7cc] bg-white p-6">
        <h2 className="text-[34px] font-semibold tracking-[-0.02em] text-[#111111]">고객 평점</h2>
        <div className="mt-3 flex items-end gap-2">
          <p className="text-[52px] leading-none font-semibold text-[#111111]">{salon.rating}</p>
          <p className="pb-1 text-lg text-[#6e665d]">/ 5</p>
        </div>
        <p className="mt-1 text-sm text-[#6e665d]">리뷰 {salon.reviewCount.toLocaleString()}개 기준</p>
        ...
        <div className="mt-5 rounded-2xl bg-[#f7f3ec] p-4">
          <p className="text-sm font-semibold text-[#433c34]">이런 점이 좋아요!</p>
          <div className="mt-2 flex flex-wrap gap-2">
            {["친절해요", "시술이 꼼꼼해요", "스타일 추천을 잘해줘요", "매장이 깔끔해요", "만족스러워요"].map((item) => (
              <span key={item} className="rounded-full border border-[#ddd4c9] bg-white px-2.5 py-1 text-xs text-[#5b5349]">
                {item}
              </span>
            ))}
          </div>
